...
class PermissionsManager(private val activity: ComponentActivity) {
    ...
    private val permissionsCheckers =
        Permission.entries.associateWith { PermissionsChecker(activity, it.toAndroidPermissions()) }

    fun permissionState(permission: Permission) =
    ...
    fun permissionStateFlow(permission: Permission) =
        activity
            .permissionsGrantedFlow(activity.lifecycle, permission.toAndroidPermissions())
            .mapLatest {
                when (it) {
                    true -> PermissionState.GRANTED
                    false -> PermissionState.NOT_GRANTED
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    suspend fun withPermissionGranted(permission: Permission, block: suspend () -> Unit) =
        permissionStateFlow(permission).distinctUntilChanged().collectLatest { permissionState ->
    ...
    private val Permission.permissionsChecker: PermissionsChecker
        get() =
            permissionsCheckers[this] ?: error("No PermissionsChecker for permission ${this.name}")
    ...
    companion object {
        ...
        private fun Permission.toAndroidPermissions() =
            when (this) {

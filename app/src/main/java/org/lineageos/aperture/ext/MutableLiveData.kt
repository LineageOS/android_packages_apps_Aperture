...
import org.lineageos.aperture.utils.livedatadelegate.LiveDataDelegate
...
inline fun <reified T> nonNullablePropertyDelegate(noinline initializer: () -> MutableLiveData<T>): LiveDataDelegate<T> =
    NonNullableLiveDataDelegate(initializer)

inline fun <reified T> nullablePropertyDelegate(noinline initializer: () -> MutableLiveData<T?>): LiveDataDelegate<T?> =

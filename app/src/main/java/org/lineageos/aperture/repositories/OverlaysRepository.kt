...
class OverlaysRepository(private val context: Context) {
    ...
    val additionalVideoConfigurations =
        buildMap<String, MutableMap<Quality, MutableMap<FrameRate, Boolean>>> {
                getStringArray(R.array.config_additionalVideoConfigurations).let {
                    if (it.size % 3 != 0) {
                    ...
                    for (i in it.indices step 3) {
                        ...
                        val frameRates =
                            it[i + 2]
                                .split("|")
                                .mapNotNull { frameRate ->
                                    FrameRate.fromValue(frameRate.toInt().absoluteValue)?.let {
                                        value ->
                        ...
                        it[i + 1]
                            ...
                            .forEach { quality ->
                                getOrCreate(cameraId).apply {
                                    getOrCreate(quality).apply { putAll(frameRates) }
                                }
                            }
                    }
                }
            }
            .map { a -> a.key to a.value.map { b -> b.key to b.value.toMap() }.toMap() }
            .toMap()

    /** @see R.array.config_logicalZoomRatios */
    val logicalZoomRatios =
        buildMap<String, MutableMap<Float, Float>> {
                getStringArray(R.array.config_logicalZoomRatios).let {
                    ...
                    for (i in it.indices step 3) {
                        ...
                        getOrCreate(cameraId).apply { this[approximateZoomRatio] = exactZoomRatio }
                    }
                }
            }
            .map { a -> a.key to a.value.toMap() }
            .toMap()
    ...
    private fun getBoolean(id: Int) =
        rroResources.firstNotNullOfOrNull { runCatching { it.getBoolean(id) }.getOrNull() }
            ?: context.resources.getBoolean(id)

    private fun getStringArray(id: Int) =
        rroResources.firstNotNullOfOrNull { runCatching { it.getStringArray(id) }.getOrNull() }

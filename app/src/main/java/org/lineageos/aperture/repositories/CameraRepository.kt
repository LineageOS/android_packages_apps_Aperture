...
class CameraRepository(
    ...
    val mainBackCamera by lazy {
        internalCameras.firstOrNull { camera -> camera.cameraFacing == CameraFacing.BACK }
    }

    val mainFrontCamera by lazy {
        internalCameras.firstOrNull { camera -> camera.cameraFacing == CameraFacing.FRONT }
    }

    /**
     * List of external cameras. These will change once the user connects or disconnects a camera.
     */
    private val externalCameras =
        suspend {
                cameraProvider.availableCameraInfos
                    .filter { cameraXCameraInfo ->
                        cameraXCameraInfo.lensFacing == CameraSelector.LENS_FACING_EXTERNAL
    ...
    val cameras =
        externalCameras
            .mapLatest { externalCameras -> internalCameras + externalCameras }
            .flowOn(Dispatchers.IO)
            .shareIn(scope = coroutineScope, started = SharingStarted.WhileSubscribed(), replay = 1)

    /** @see ExtensionsManager.getExtensionEnabledCameraSelector */
    fun getExtensionEnabledCameraSelector(camera: Camera, mode: Int): CameraSelector =
        extensionsManager.getExtensionEnabledCameraSelector(camera.cameraSelector, mode)
    ...
    private fun CameraInfo.isInternalCameraAllowed(): Boolean {
        ...
        return when (camera2CameraInfo.cameraId) {
            in mainCameraIds -> true
                ...
                val isIgnoredAuxCamera =
                    overlaysRepository.ignoredAuxCameraIds.contains(camera2CameraInfo.cameraId)
                val isIgnoredLogicalCamera =
                    overlaysRepository.ignoreLogicalAuxCameras && physicalCameraInfos.size > 1
    ...
    private fun CameraInfo.toCamera() =
        Camera.fromCameraX(this, extensionsManager, overlaysRepository)

    private fun requireCameraPermission() =
        require(context.permissionGranted(Manifest.permission.CAMERA)) {
            "Camera permission not granted"
        }

    companion object {
        /** List of main camera IDs. These should never be excluded. */
        private val mainCameraIds = setOf("0", "1")
    }
}

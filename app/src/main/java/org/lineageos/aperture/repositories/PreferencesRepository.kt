...
class PreferencesRepository(context: Context, private val coroutineScope: CoroutineScope) {
    /** [Preference] holder. */
    inner class PreferenceHolder<T>(private val preference: Preference<T>) :
        Flow<T> by preference.preferenceFlow() {
        /** The value. */
        var value: T
            get() = preference.getValue(sharedPreferences)
            set(value) = sharedPreferences.edit { preference.setValue(this, value) }
    }

    private val sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context)

    /** The last [CameraFacing] used. */
    val lastCameraFacing =
        enumPreference(
    ...
    val videoMicMode =
        primitivePreference(key = "last_mic_mode", defaultValue = true).asPreferenceHolder()
    ...
    val enableZsl =
        primitivePreference(key = "enable_zsl", defaultValue = false).asPreferenceHolder()

    /** Whether JPEG Ultra HDR should be used to store images if available. */
    val usePhotoJpegUltraHdr =
        primitivePreference(key = "use_photo_jpeg_ultra_hdr", defaultValue = true)
            .asPreferenceHolder()

    /** Enable RAW image capture. */
    val enableRawImageCapture =
        primitivePreference(key = "enable_raw_image_capture", defaultValue = false)
            .asPreferenceHolder()

    /** Whether storing JPEG should be disabled when using RAW. */
    val disableJpegWithRaw =
        primitivePreference(key = "disable_jpeg_with_raw", defaultValue = false)
            .asPreferenceHolder()

    /** Toggle left-right mirroring of the front facing camera images. */
    val photoFfcMirror =
        primitivePreference(key = "photo_ffc_mirror", defaultValue = true).asPreferenceHolder()
    ...
    val photoAspectRatio =
        enumPreference<Int, String>(
                key = "aspect_ratio",
                defaultValue = AspectRatio.RATIO_4_3,
                enumToPreferenceValue =
                    mapOf(AspectRatio.RATIO_4_3 to "4_3", AspectRatio.RATIO_16_9 to "16_9"),
            )
            .asPreferenceHolder()

    /** Whether bright screen should be enabled. */
    val brightScreen =
        primitivePreference(key = "bright_screen", defaultValue = false).asPreferenceHolder()

    /** Whether location should be saved. */
    val saveLocation =
        primitivePreference<Boolean?>(key = "save_location", defaultValue = null)
            .asPreferenceHolder()

    /** Whether shutter sound should be played. */
    val shutterSound =
        primitivePreference(key = "shutter_sound", defaultValue = true).asPreferenceHolder()

    /** Whether leveler should be visible. */
    val leveler = primitivePreference(key = "leveler", defaultValue = false).asPreferenceHolder()

    /** Whether video stabilization should be enabled. */
    val videoStabilization =
        primitivePreference(key = "video_stabilization", defaultValue = true).asPreferenceHolder()
    ...
    val forceTorchHelpShown =
        primitivePreference(key = "force_torch_help_shown", defaultValue = false)
            .asPreferenceHolder()
    ...
    val hardwareKeyActionPreferences =
        HardwareKey.entries.associateWith { hardwareKey ->
            enumPreference(
    ...
    val hardwareKeyInvertPreferences =
        HardwareKey.entries.associateWith {
            primitivePreference(
    ...
    private fun <T> Preference<T>.preferenceFlow() =
        sharedPreferences
            .preferenceFlow(key, getter = { getValue(sharedPreferences) })
            .flowOn(Dispatchers.IO)
            .shareIn(scope = coroutineScope, started = SharingStarted.WhileSubscribed(), replay = 1)

    private fun <T> Preference<T>.asPreferenceHolder() = PreferenceHolder(preference = this)
    ...
    companion object {
        ...
        fun GestureAction.toPreferenceString() =
            when (this) {
        ...
        fun preferenceStringToGestureAction(string: String?) =
            when (string) {
                ...
                "volume",
                "default" -> GestureAction.DEFAULT // volume for compat

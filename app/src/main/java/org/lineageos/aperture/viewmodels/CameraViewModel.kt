...
class CameraViewModel(application: Application) : ApertureViewModel(application) {
    ...
    /**
     * Whether the record audio permission has been granted by the user.
     */
    val isRecordAudioPermissionGranted = MutableStateFlow(true)
    ...
    fun initializeCameraConfiguration(): Boolean {
        ...
        val camera = getCameraOfFacingOrFirstAvailable(
    ...
    ) = cameraRepository.cameras.value.filter { camera -> // TODO: Race-y
        camera.cameraFacing == cameraFacing && when (cameraMode) {
    ...
    fun getCameraOfFacingOrFirstAvailable(
        cameraMode: CameraMode,
        cameraFacing: CameraFacing,
    ) = getCameras(cameraMode, cameraFacing).firstOrNull()
        ...
        cameraController.takePicture(
            ...
            object : ImageCapture.OnImageSavedCallback {
                ...
                override fun onImageSaved(output: ImageCapture.OutputFileResults) {
                    ...
                    if (!inSingleCaptureMode.value) {
                        output.savedUri?.let {
                            BroadcastUtils.broadcastNewPicture(applicationContext, it)
                        }
                    }
                }
    ...
    fun setVideoMicrophoneEnabled(videoMicrophoneEnabled: Boolean) {
        videoAudioConfig = when (isRecordAudioPermissionGranted.value) {
            true -> AudioConfig.create(videoMicrophoneEnabled)
    ...
    private fun createInitialCameraConfiguration(
        ...
        return when (cameraMode) {
            CameraMode.PHOTO -> CameraConfiguration.Photo(
                camera = camera,
                extensionMode = preferencesRepository.photoEffect.value,
                photoCaptureMode = preferencesRepository.photoCaptureMode.value,

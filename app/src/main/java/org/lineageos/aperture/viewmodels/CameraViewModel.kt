...
class CameraViewModel(application: Application) : ApertureViewModel(application) {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    private val exposureCompensationInfo = camera
    ...
    private val exposureCompensationIndex = exposureCompensationInfoToIndex
        .mapLatest { exposureCompensationInfoToIndex ->
            exposureCompensationInfoToIndex?.second ?: 0
        }

    /**
     * The current exposure compensation EV value.
     */
    @OptIn(ExperimentalCoroutinesApi::class)
    private val exposureCompensationValue = exposureCompensationInfoToIndex
        .mapLatest { exposureCompensationInfoToIndex ->
            exposureCompensationInfoToIndex?.let {
                it.first.getExposureValue(it.second)
            } ?: 0f
        }
        .flowOn(Dispatchers.IO)
        .shareIn(

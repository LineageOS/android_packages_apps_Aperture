...
open class CameraActivity : AppCompatActivity(R.layout.activity_camera) {
            ...
            it.onResultCallback = { result ->
                if (result) {
                    lifecycleScope.launch {
                        ...
                        sharedPreferences.saveLocation = permissionState == PermissionState.GRANTED
                    }
                } else {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            lifecycle.repeatOnLifecycle(Lifecycle.State.RESUMED) {
                val permissionState = permissionsManager.permissionState(Permission.CAMERA)
            ...
            camera2CameraControl.captureRequestOptions = CaptureRequestOptions.Builder()
                ...
                .setEdgeMode(
                    sharedPreferences.edgeMode?.takeIf {
                        cameraConfiguration.camera.supportedEdgeModes.contains(it) && when (cameraConfiguration) {
                            is CameraConfiguration.Photo -> cameraConfiguration.photoCaptureMode !=
                                    ImageCapture.CAPTURE_MODE_ZERO_SHUTTER_LAG ||
                                    EdgeMode.ALLOWED_MODES_ON_ZSL.contains(it)

                            is CameraConfiguration.Video -> EdgeMode.ALLOWED_MODES_ON_VIDEO_MODE.contains(
                                it
                            )
                ...
                .setNoiseReductionMode(
                    sharedPreferences.noiseReductionMode?.takeIf {
                        cameraConfiguration.camera.supportedNoiseReductionModes.contains(it) && when (cameraConfiguration) {
                            is CameraConfiguration.Photo -> cameraConfiguration.photoCaptureMode !=
                                    ImageCapture.CAPTURE_MODE_ZERO_SHUTTER_LAG ||
                ...
                .setShadingMode(
                    sharedPreferences.shadingMode?.takeIf {
                        cameraConfiguration.camera.supportedShadingModes.contains(it) && when (cameraConfiguration) {
                            is CameraConfiguration.Photo -> cameraConfiguration.photoCaptureMode !=
                                    ImageCapture.CAPTURE_MODE_ZERO_SHUTTER_LAG ||
                ...
                .setColorCorrectionAberrationMode(
                    sharedPreferences.colorCorrectionAberrationMode?.takeIf {
                        cameraConfiguration.camera.supportedColorCorrectionAberrationModes.contains(
                            it
                        ) && when (cameraConfiguration) {
                            is CameraConfiguration.Photo -> cameraConfiguration.photoCaptureMode !=
                                    ImageCapture.CAPTURE_MODE_ZERO_SHUTTER_LAG ||
                ...
                .setDistortionCorrectionMode(
                    sharedPreferences.distortionCorrectionMode?.takeIf {
                        cameraConfiguration.camera.supportedDistortionCorrectionModes.contains(it) && when (cameraConfiguration) {
                            is CameraConfiguration.Photo -> cameraConfiguration.photoCaptureMode !=
                                    ImageCapture.CAPTURE_MODE_ZERO_SHUTTER_LAG ||
                ...
                .setHotPixelMode(
                    sharedPreferences.hotPixelMode?.takeIf {
                        cameraConfiguration.camera.supportedHotPixelModes.contains(it) && when (cameraConfiguration) {
                            is CameraConfiguration.Photo -> cameraConfiguration.photoCaptureMode !=
                                    ImageCapture.CAPTURE_MODE_ZERO_SHUTTER_LAG ||
    ...
    private fun changeCameraMode(cameraMode: CameraMode) {
        ...
        viewModel.setCameraMode(cameraMode)

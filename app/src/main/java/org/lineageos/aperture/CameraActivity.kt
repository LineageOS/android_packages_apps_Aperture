...
open class CameraActivity : AppCompatActivity(R.layout.activity_camera) {
    ...
    private val supportedFlashModes: Set<FlashMode>
        get() = viewModel.cameraMode.value.supportedFlashModes.intersect(
            viewModel.camera.value?.supportedFlashModes.orEmpty()
        )
    ...
    private val orientationEventListener by lazy {
        object : OrientationEventListener(this) {
            override fun onOrientationChanged(orientation: Int) {
                ...
                if (viewModel.screenRotation.value != rotation) {
                    viewModel.screenRotation.value = rotation
                }
            }
        }
    }
    ...
    private val batteryBroadcastReceiver by lazy {
        object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                infoChipView.setBatteryIntent(intent)
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.videoQuality.value = sharedPreferences.videoQuality
        viewModel.videoFrameRate.value = sharedPreferences.videoFrameRate
        viewModel.videoDynamicRange.value = sharedPreferences.videoDynamicRange
        viewModel.videoMicMode.value = sharedPreferences.lastMicMode
        ...
        viewModel.camera.value = viewModel.getCameraOfFacingOrFirstAvailable(
            initialCameraFacing, viewModel.cameraMode.value
        ) ?: run {
            noCamera()
            return
        }
        ...
        viewModel.cameraController.torchState.observe(this) {
            viewModel.flashMode.value = viewModel.cameraController.flashMode
        }

        // Observe focus state
        viewModel.cameraController.tapToFocusState.observe(this) {
            when (it) {
    ...
    private fun captureVideo() {
        ...
        val outputOptions = StorageUtils.getVideoMediaStoreOutputOptions(
            contentResolver,
            location.takeUnless { viewModel.inSingleCaptureMode.value }
        )
    ...
    private fun bindCameraUseCases() {
        ...
        var camera = when (viewModel.cameraMode.value) {
            CameraMode.QR -> viewModel.getCameraOfFacingOrFirstAvailable(
                CameraFacing.BACK, viewModel.cameraMode.value
            )
        ...
        if (!camera.supportsCameraMode(viewModel.cameraMode.value)) {
            camera = viewModel.getCameraOfFacingOrFirstAvailable(
                camera.cameraFacing, viewModel.cameraMode.value
            ) ?: run {
                noCamera()
                return
            }
    ...
    private fun cycleFlashMode(forceTorch: Boolean): Boolean {
        ...
        val forceTorchAvailable = viewModel.cameraMode.value == CameraMode.PHOTO
                && viewModel.cameraMode.value.supportedFlashModes.contains(FlashMode.TORCH)

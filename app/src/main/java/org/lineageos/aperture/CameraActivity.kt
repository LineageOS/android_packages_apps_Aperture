...
open class CameraActivity : AppCompatActivity(R.layout.activity_camera) {
    ...
    private fun bindCameraUseCases() {
        ...
        viewModel.photoEffect.value = listOf(
            ...
            println(it)
            camera.supportsExtensionMode(it).also {
                println("Supported: $it")
            }
        } ?: ExtensionMode.NONE
        ...
        lifecycleScope.launch {
            ...
            camera2CameraControl.captureRequestOptions = CaptureRequestOptions.Builder()
                .setFrameRate(
                    viewModel.videoFrameRate.value.takeIf {
                        viewModel.cameraMode.value == CameraMode.VIDEO
                    }
                )
                .setVideoStabilizationMode(
                    if (viewModel.cameraMode.value == CameraMode.VIDEO && sharedPreferences.videoStabilization) {

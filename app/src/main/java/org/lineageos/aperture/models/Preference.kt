...
sealed interface Preference<T> {
    ...
    sealed class PrimitivePreference<T>(
        ...
        override fun getValue(sharedPreferences: SharedPreferences) =
    ...
    /** [Boolean] preference. */
    class BooleanPreference<T : Boolean?>(key: String, defaultValue: T) :
    ...
    /** [Float] preference. */
    class FloatPreference<T : Float?>(key: String, defaultValue: T) :
        PrimitivePreference<T>(
            key,
            defaultValue,
    ...
    /** [Int] preference. */
    class IntPreference<T : Int?>(key: String, defaultValue: T) :
        PrimitivePreference<T>(
            key,
            defaultValue,
    ...
    /** [Long] preference. */
    class LongPreference<T : Long?>(key: String, defaultValue: T) :
        PrimitivePreference<T>(
            key,
            defaultValue,
    ...
    /** [String] preference. */
    class StringPreference<T : String?>(key: String, defaultValue: T) :
        PrimitivePreference<T>(
            key,
            defaultValue,
    ...
    class EnumPreference<T, BT>(
        ...
        private val preferenceValueToEnum: Map<BT, T> =
            enumToPreferenceValue.entries.associate { it.value to it.key }
    ...
    companion object {
        @Suppress("UNCHECKED_CAST") private fun <T> Any.forceCast() = this as T

        @Suppress("UNCHECKED_CAST")
        inline fun <reified T> primitivePreference(key: String, defaultValue: T): Preference<T> =
            when (T::class) {
                Boolean::class ->
                    when (null is T) {
        ...
        inline fun <reified T, reified BT> enumPreference(
            ...
            EnumPreference(
                ...
                enumToPreferenceValue =
                    buildMap {
                        enumValues.forEach { put(it, enumMapper(it)) }

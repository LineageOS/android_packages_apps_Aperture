...
data class AssistantIntent(
    ...
    companion object {
        ...
        fun fromIntent(intent: Intent): AssistantIntent? {
            ...
            val useFrontCamera =
                EXTRA_USE_FRONT_CAMERA.firstOrNull { extras.containsKey(it) }
                    ?.let { extras.getBoolean(it) }
            val cameraOpenOnly =
                EXTRA_CAMERA_OPEN_ONLY.firstOrNull { extras.containsKey(it) }
                    ?.let { extras.getBoolean(it) }
            val timerDurationSeconds =
                EXTRA_TIMER_DURATION_SECONDS.firstOrNull { extras.containsKey(it) }

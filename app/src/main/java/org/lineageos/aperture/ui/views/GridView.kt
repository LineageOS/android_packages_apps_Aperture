...
class GridView(context: Context?, attributeSet: AttributeSet?) : View(context, attributeSet) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val surface =
            previewView.children.firstOrNull { it is TextureView || it is SurfaceView }
        ...
        for (i in size - 1 downTo 1) {
            val position =
                if (mode == GridMode.ON_GOLDEN_RATIO && i == 2) 1 + GOLDEN_RATIO else i.toFloat()

            canvas.drawLine(
                (widthSection * position) + wOffset,
                hOffset,
                (widthSection * position) + wOffset,
                height + hOffset,
                paint,
            ...
            canvas.drawLine(
                wOffset,
                (heightSection * position) + hOffset,
                width + wOffset,
                (heightSection * position) + hOffset,
                paint,
            )

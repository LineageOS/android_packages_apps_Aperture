...
class QrTextClassifier(private val context: Context) {
    ...
    fun classify(result: BarcodeReader.Result): QrResult {
                    ...
                    if (
                        ...
                        return QrResult(context) {
                            ...
                            addAction {
                                ...
                                setIntent(
                                    Intent(Settings.ACTION_PROCESS_WIFI_EASY_CONNECT_URI, uri)
                                )
                            }
                        }
                    }
        ...
        return textClassificationManager.textClassifier
            .classifyText(spannableString, 0, spannableString.length, LocaleList.getDefault())
            .let {
                QrResult(context) {
                    text = it.text
    ...
    private fun BarcodeReader.Result.toResult() =
        Result(
            text,
            bytes,
            null,
    ...
    fun AddressBookParsedResult.toQrResult(context: Context) =
        QrResult(context) {
            ...
            addAction {
                ...
                setIntent(
                    Intent(Intent.ACTION_INSERT, ContactsContract.Contacts.CONTENT_URI).apply {
                        ...
                        phoneNumbers?.let { phoneNumbers ->
                            ...
                            for ((i, keys) in
                                ...
                                phoneNumbers.getOrNull(i)?.let { phone ->
                                    ...
                                    phoneTypes.getOrNull(i)?.let { putExtra(keys.last(), it) }
                                }
                            }
                        }
                        ...
                        emails?.let { emails ->
                            ...
                            for ((i, keys) in
                                ...
                                emails.getOrNull(i)?.let { phone ->
                                    putExtra(keys.first(), phone)
                                    emailTypes.getOrNull(i)?.let { putExtra(keys.last(), it) }
                                }
                            }
                        }
                        ...
                        note?.let { putExtra(ContactsContract.Intents.Insert.NOTES, it) }
                        ...
                        addresses?.let { emails ->
                            ...
                            for ((i, keys) in
                                ...
                                emails.getOrNull(i)?.let { phone ->
                                    putExtra(keys.first(), phone)
                                    addressTypes.getOrNull(i)?.let { putExtra(keys.last(), it) }
                                }
                            }
                        }
    ...
    fun CalendarParsedResult.toQrResult(context: Context) =
        QrResult(context) {
            text = summary
            ...
            addAction {
                ...
                setIntent(
                    Intent(Intent.ACTION_INSERT, CalendarContract.Events.CONTENT_URI).apply {
                        ...
                        organizer?.let { putExtra(CalendarContract.Events.ORGANIZER, it) }
                        attendees?.let { putExtra(Intent.EXTRA_EMAIL, it.joinToString(",")) }

                        putExtras(
                            bundleOf(
                                CalendarContract.EXTRA_EVENT_BEGIN_TIME to startTimestamp,
                                CalendarContract.EXTRA_EVENT_END_TIME to endTimestamp,
                                CalendarContract.EXTRA_EVENT_ALL_DAY to
                                    (isStartAllDay && isEndAllDay),
                            )
                        )
                    }
                )
    ...
    fun EmailAddressParsedResult.toQrResult(context: Context) =
        QrResult(context) {
            ...
            addAction {
                ...
                setIntent(
                    Intent(Intent.ACTION_SENDTO, Uri.parse("mailto:${tos?.firstOrNull() ?: ""}"))
                        .apply {
                            putExtras(
                                bundleOf(
    ...
    fun GeoParsedResult.toQrResult(context: Context) =
        QrResult(context) {
            text = displayResult
    ...
    fun ISBNParsedResult.toQrResult(context: Context) =
        QrResult(context) {
            text = isbn
            ...
            addAction {
                ...
                setIntent(
                    Intent(Intent.ACTION_VIEW, Uri.parse("https://isbnsearch.org/isbn/${isbn}"))
                )
            }
        }

    fun ProductParsedResult.toQrResult(context: Context) =
        QrResult(context) {
            text = productID
    ...
    fun SMSParsedResult.toQrResult(context: Context) =
        QrResult(context) {
            text = numbers.first()
    ...
    fun TelParsedResult.toQrResult(context: Context) =
        QrResult(context) {
            text = number
    ...
    fun VINParsedResult.toQrResult(context: Context) =
        QrResult(context) {
            text = vin
    ...
    fun WifiParsedResult.toQrResult(context: Context) =
        QrResult(context) {
            text = ssid
                                        ...
                                        .apply {
                                            password?.let {
                                                when (networkEncryption) {
                                                    "WPA" -> {
                                                        // Per specs, Wi-Fi QR codes are only used
                                                        // for
                                                        // WPA2 and WPA-Mixed networks, we can
                                                        // safely

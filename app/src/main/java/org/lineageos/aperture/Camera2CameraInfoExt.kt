...
val LOGICAL_MULTI_CAMERA_PHYSICAL_IDS: CameraCharacteristics.Key<ByteArray> =
    CameraCharacteristics.Key("android.logicalMultiCamera.physicalIds",
...
val Camera2CameraInfo.physicalCameraIds: Set<String>
    ...
    get() {
        val availableCapabilities = getCameraCharacteristic(
            CameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES) ?: throw AssertionError(
            "android.request.availableCapabilities must be non-null "
        ...
        val physicalCamIds: ByteArray = getCameraCharacteristic(
            LOGICAL_MULTI_CAMERA_PHYSICAL_IDS) ?: throw AssertionError(
            "android.logicalMultiCamera.physicalIds must be non-null "
                    + "in the characteristics"
            )
